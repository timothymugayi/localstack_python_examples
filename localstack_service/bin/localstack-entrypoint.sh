...
export LOCALSTACK_S3_URL=http://localhost:4572
export LOCALSTACK_SQS_URL=http://localhost:4576
...
#printf "Download Sample data begin..."
...
#aws --endpoint-url=$LOCALSTACK_S3_URL s3 ls s3://nyc-tlc
## create tmp directory to put sample data after chunking
mkdir -p /tmp/localstack/taxi_data
...
#aws --endpoint-url=$LOCALSTACK_S3_URL s3 sync /tmp/localstack/taxi_data "s3://nyc-tlc/trip data" --cli-connect-timeout 6000
